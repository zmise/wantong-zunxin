        ...
        if (items.code == 'ok') {
          ...
          $.each(items.data, function (k, v) {
            if (k == 'registrationAreaName') {
              ...
              if (v == 1) {
                statusHtml = '未预约';
                $publicBtn.text('预约取号').attr('href', '/reservation-code.html?id=' + id + '');
              } else if (v == 2) {
                statusHtml = '已预约';
                if (timeFlag) {
                  statusHtml = '已过期';
                  $publicBtn.text('重新预约').attr('href', '/reservation-code.html?id=' + id + '');
                } else {
    ...
    if (self.data('type') == 'cancel') {
      $.ajax({
        ...
        success: function (data) {
          $.toast(data.msg);
          $('.status').text('已取消');
          $publicBtn.text('重新预约').attr('href', '/reservation-code.html?id=' + self.data('id') + '');
        }
      })
    }
  })
